...
const DatabasePage = () => {
  ...
  const fetchVectorStoreFiles = useCallback(async () => {
    console.log("fetchVectorStoreFiles: Called. User:", user?.id, "ActiveClass:", activeClass);
    ...
    console.log(`fetchVectorStoreFiles: Fetching for VS ID: ${activeClass.openAIConfig.vectorStoreId}`);
    try {
      ...
      if (functionError) {
        console.error("fetchVectorStoreFiles: Edge function error", functionError);
        throw functionError;
      }
      if (data.error) {
      ...
      if (openAIFileIds.length > 0) {
        ...
        if (dbError) {
          console.error("fetchVectorStoreFiles: Error fetching file details from DB for vector store list:", dbError);
        } else if (filesFromDB) {
  ...
  const confirmVectorUpload = async () => {
    ...
    try {
      ...
      if (functionError) {
        console.error("confirmVectorUpload: Edge function invocation error:", functionError);
        throw functionError;
      }
      ...
      if (functionResponse.results && Array.isArray(functionResponse.results)) {
        for (const result of functionResponse.results) {
          if (result.success && result.openAiFileId && result.internal_file_id) {
            successCount++;
            console.log(`confirmVectorUpload: Updating file ${result.internal_file_id} with OpenAI File ID ${result.openAiFileId}`);
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
          ...
          <TabsContent value="myFiles" className="mt-6">
            <div className="flex flex-wrap gap-2 mb-6 mt-4 items-center">
              ...
              ) : ( <> <Button onClick={() => setIsVectorUploadDialogOpen(true)} disabled={selectedItems.filter(item => item.type === 'file' && item.url).length === 0}> <ArrowUp className="h-4 w-4 mr-2" /> Push to Vector Store ({selectedItems.filter(item => item.type === 'file' && item.url).length}) </Button> <Button variant="outline" onClick={() => { setSelectionMode(false); setSelectedItems([]); }}> Cancel Selection </Button> </> )}
            </div>
