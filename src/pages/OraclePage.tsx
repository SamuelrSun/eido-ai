...
const OraclePage = () => {
        ...
        const fetchUserAndClasses = async () => {
            ...
            if (currentUser) {
                ...
                if (activeClassDataString) {
                    try {
                        const parsedClass: { class_id: string } = JSON.parse(activeClassDataString);
                        ...
                        if (fetchedClasses.some(c => c.class_id === parsedClass.class_id)) {
                            ...
                            console.log(`OraclePage: Automatically selected active class "${parsedClass.class_id}" from session.`);
                        }
                    } catch (e) {
    ...
    }, [user, toast, selectedClassId, selectedConversationId]); // Re-fetch when selectedClassId changes

    useEffect(() => { fetchConversations(); }, [user, fetchConversations]); // Add fetchConversations to dependency array
        ...
        try {
            ...
            if (shouldGenerateTitle) {
                try {
                    ...
                    if (data && data.title) {
                        await handleRenameConversation(activeConversationId!, data.title);
                    }
                } catch (titleError) {
            ...
            if (aiError || !aiData) { throw new Error(aiError?.message || "Invalid response from AI service."); }

            const savedAiMessage = await chatMessageService.saveMessage({ 
                conversation_id: activeConversationId!, role: 'assistant', 
                content: aiData.response, chat_mode: 'rag', class_id: selectedClassId,
            ...
            toast({ title: "Error", description: `Failed to get a response: ${(error as Error).message}`, variant: "destructive" });
    ...
    const handleRenameConversation = async (id: string, newName: string) => {
        ...
        try { 
         ...
         catch (error) { toast({ title: "Error", description: "Failed to rename chat.", variant: "destructive" }); fetchConversations(); }
    };
                                         ...
                                         <Select onValueChange={(value) => {
                                            ...
                                            setMessages([]); // Clear messages when dataset changes
                                             ...
                                             <SelectContent>
                                                 ...
                                                 {classes.map(cls => (<SelectItem key={cls.class_id} value={cls.class_id}>{cls.title}</SelectItem>))}
                                             </SelectContent>
                                         </Select>
                                         </div>
                                 </div>
