...
serve(async (req) => {
  ...
  try {
    ...
    let assistantContent = textContent.text.value;
    
    // Remove any source citations that might still be present
    assistantContent = assistantContent.replace(/【\d+:\d+†source】/g, "");
