// supabase/functions/approve-class-member/index.ts
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (classData.owner_id !== requestingUser.id) {
        return new Response(JSON.stringify({ error: "Only the class owner can approve members." }), {
            status: 403, // Forbidden
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
    }

    // Update the member's role from 'pending' to 'member'
    const { error: updateError } = await adminSupabaseClient
      .from('class_members')
      .update({ role: 'member' })
      .eq('class_id', class_id)
      .eq('user_id', member_id)
      .eq('role', 'pending'); // Ensure we only approve pending members

    if (updateError) throw updateError;

    return new Response(JSON.stringify({ message: "Member approved successfully." }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
